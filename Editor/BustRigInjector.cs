...
public class BustRigInjector : EditorWindow
{
    private AnimatorController targetController;
    private VRCExpressionParameters selectedExpParams;

    private Transform leftBustBone;
    private Transform rightBustBone;

    private string clipOutputFolder = "Assets/Animations/BustTracking";
    ...
    private const string LeftPitch = "LeftBustPitch";
    private const string LeftYaw = "LeftBustYaw";
    private const string RightPitch = "RightBustPitch";
    private const string RightYaw = "RightBustYaw";
    private const string BustVertical = "BustVertical";
    ...
    private const string PrefPrefix = "VRCSlimeVRBustRigSupport_";

    [MenuItem("Tools/Bust Rig/Add Bust Tracking Compatibility")]
    public static void Open()
    {
        GetWindow<BustRigInjector>("Bust Tracking Configurator");
    }
    ...
    private void OnEnable()
    {
        ...
        clipOutputFolder = EditorPrefs.GetString(
            ...
            "Assets/Animations/BustTracking"
        );
        ...
        leftBustBone = LoadTransformPreference(PrefPrefix + "LeftBustBone");
        rightBustBone = LoadTransformPreference(PrefPrefix + "RightBustBone");
    }
    ...
    private void OnDisable()
    {
        ...
        SaveTransformPreference(
            PrefPrefix + "LeftBustBone",
            leftBustBone
        );

        SaveTransformPreference(
            PrefPrefix + "RightBustBone",
            rightBustBone
        );
    }
    ...
    private void OnGUI()
    {
        ...
        EditorGUILayout.LabelField(
            "SlimeVR Bust Tracking Configurator",
        ...
        if (GUILayout.Button("Generate Bust Support"))
        {
            Generate();
        }
    ...
    private void Generate()
    {
        ...
        EditorUtility.DisplayDialog(
            "Bust configuration completed!",
            "Bust tracking support created successfully!",
            "OK"
        );
    }

    private void GenerateBust(
        string sideName,
        Transform bone,
        ...
        string yawParameter)
    {
        ...
        {
            Debug.LogError(
                "[BustRig] The selected AnimatorController is not a saved asset."
            );
            return;
        }
        ...
        Debug.Log(
            $"[BustRig] {sideName}: RAW OSC Float -> ParameterDriver 4-bit synced Bool encoder; " +
    ...
    private void GenerateSharedVertical()
    {
        ...
        RemoveExistingLayer("BustVertical");
        RemoveExistingLayer("BustVertical_4BitEncode");
        RemoveExistingLayer("BustVertical_4BitDecode");
        ...
        GenerateFourBitEncoderLayer(
            "BustVertical_4BitEncode",
            BustVertical,
            controllerPath
        );

        GenerateFourBitDecoderLayer(
            "BustVertical_4BitDecode",
            BustVertical,
            controllerPath
        );
        ...
        AnimationClip verticalNegative =
            CreateWorldRelativeVerticalClip(
                "BustVertical_Negative",
                -maxVertical
            );

        AnimationClip verticalNeutral =
            CreateWorldRelativeVerticalClip(
                "BustVertical_Neutral",
                0f
            );

        AnimationClip verticalPositive =
            CreateWorldRelativeVerticalClip(
                "BustVertical_Positive",
                maxVertical
            );
        ...
        Debug.Log(
            "[BustRig] Shared BustVertical generated: local Float encoder, " +
        ...
        float verticalMeters)
    {
        ...
        {
            Debug.LogWarning(
                $"[BustRig] Could not determine an animation path for '{bone.name}'."
            );
            return;
        }
        ...
        {
            Debug.LogWarning(
                $"[BustRig] Neutral vertical pose for '{bone.name}' did not match rest position."
            );
        }
    }
        ...
        string parameter)
    {
        ...
        {
            Debug.LogWarning(
                $"[BustRig] Rebuilding Animator parameter '{parameter}' as Bool. " +
        ...
        string parameter)
    {
        ...
        {
            Debug.LogError(
                $"[BustRig] Animator parameter '{parameter}' already exists " +
        ...
        string controllerPath)
    {
        ...
        {
            Debug.LogError(
                $"[BustRig] Refusing to encode OSCmooth proxy '{rawSourceFloatParameter}'. " +
        ...
        Debug.Log(
            $"[BustRig] Generated ParameterDriver encoder '{layerName}' from raw Float " +
        ...
        string controllerPath)
    {
        ...
        Debug.Log(
            $"[BustRig] Generated Bool-state ParameterDriver decoder '{layerName}' into raw Float " +
            $"'{decodedFloatParameter}'."
        );
    }
    ...
    private void AutoFillBones()
    {
            ...
            in transforms)
        {
            ...
            bool bustLike =
                normalized.Contains("bust") ||
                normalized.Contains("breast") ||
                normalized.Contains("boob");
                ...
                leftBustBone == null)
            {
                ...
                Debug.Log(
                    $"[BustRig] Auto-mapped {t.name} -> Left Bust"
                );
            } else if (
                  right &&
                  rightBustBone == null)
            {
                ...
                Debug.Log(
                    $"[BustRig] Auto-mapped {t.name} -> Right Bust"
                );
            }
        }
        ...
        string fullPath)
    {
        ...
        {
            Debug.LogError(
                $"[BustRig] Invalid animation path: {fullPath}"
            );
            return null;
        }
        ...
        string folder)
    {
        ...
        {
            Debug.LogError(
                $"[BustRig] Folder must be inside Assets: {folder}"
            );
            return;
        }
